...
from semseg.models.segformer.seg_block import Seg
...
def main(cfg):
    with torch.no_grad():
        ...
        eval_path = os.path.join(os.path.dirname(eval_cfg['MODEL_PATH']), 'eval_{}.txt'.format(cfg['DATASET']['MODALS']))
        ...
        for case in cases:
            ...
            model = Seg(backbone='mit_b0', pretrained=False, modals=cfg['DATASET']['MODALS']).to(device)
            # model = eval(cfg['MODEL']['NAME'])(cfg['MODEL']['BACKBONE'], dataset.n_classes, cfg['DATASET']['MODALS'])
            # model_path = '/home/jinjing/zhengxu/DELIVER/eccv/0220_deliver_b0_RD_all/DELIVER_CMNeXt-B2_id/CMNeXt_CMNeXt-B2_DELIVER_epoch239_63.74.pth'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./configs/seg_block/R.yaml')
